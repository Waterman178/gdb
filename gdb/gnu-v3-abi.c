...
{
  struct gdbarch *gdbarch;
  const gdb_byte *contents = method_ptr->contents ();
  CORE_ADDR ptr_value;
  ...
  method_type = TYPE_TARGET_TYPE (check_typedef (method_ptr->type ()));
...
{
  struct gdbarch *gdbarch = get_type_arch (type_info_ptr->type ());
