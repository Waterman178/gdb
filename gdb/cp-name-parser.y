...
%define api.prefix {cpname_}
%param {struct cpname_state *state}
...
int cpname_parse (void);
union YYSTYPE;
 static int cpname_lex (YYSTYPE *, cpname_state *);
static void cpname_error (cpname_state *, const char *);
...
static int
cpname_lex (YYSTYPE *lvalp, cpname_state *state)
{
  int c;
  int namelen;
  const char *tokstart;
