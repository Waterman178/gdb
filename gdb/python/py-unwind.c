              ...
              void **cache_ptr)
{
...

  {
	...
	cached_frame->reg[i].num = reg->number;
        ...
        gdb_assert (data_size == TYPE_LENGTH (value->type ()));

	cached_frame->reg[i].data = (gdb_byte *) xmalloc (data_size);
        memcpy (cached_frame->reg[i].data, value->contents (), data_size);
      }
  }
