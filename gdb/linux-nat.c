  ...
  if (lp->stopped)
	       ...
	       && lp->waitstatus.kind == TARGET_WAITKIND_IGNORE)
	{
	  ...
	  linux_ops->to_resume (find_target_ops (linux_ops),
				pid_to_ptid_target (ptid_get_lwp (lp->ptid),
						    target_stack_id ()),
				step, signo);
	  lp->stopped = 0;
	  lp->step = step;
		  ...
		  ptid_t ptid, int step, enum gdb_signal signo)
{
  ...
  ptid = pid_to_ptid_target (ptid_get_lwp (lp->ptid), target_stack_id ());
...
linux_nat_filter_event (int lwpid, int status, int *new_pending_p)
{
  ...
  *new_pending_p = 0;
  ...
  if ((WIFEXITED (status) || WIFSIGNALED (status))
	...
	{
	  lp->stopped = 1;
	  iterate_over_lwps (pid_to_ptid_target (ptid_get_pid (lp->ptid),
						 target_stack_id ()),
  ...
  if (num_lwps (ptid_get_pid (lp->ptid)) > 1 && !linux_thread_alive (lp->ptid))
    {
      ptid_t ptid = pid_to_ptid_target (ptid_get_pid (lp->ptid),
					target_stack_id ());
...
linux_nat_kill (struct gdb_target *ops)
{
  ...
  if (forks_exist_p ())
    ...
    {
      ptid_t ptid = pid_to_ptid_target (ptid_get_pid (inferior_ptid),
					target_stack_id ());
			...
			ULONGEST offset, LONGEST len)
{
  ...
  if (ptid_lwp_p (inferior_ptid))
    inferior_ptid = pid_to_ptid_target (ptid_get_lwp (inferior_ptid),
					target_stack_id ());
...
linux_child_static_tracepoint_markers_by_strid (const char *strid)
{
  ...
  char *p = s;
  ptid_t ptid = ptid_build_target (pid, 0, 0, target_stack_id ());
