			  ...
			  int regnum)
{
  ...
  if (regnum == gdbarch_sp_regnum (gdbarch))
  ...
  else if (regnum == IA64_BSP_REGNUM)
    {
      ...
      prev_cfm = extract_unsigned_integer (val->contents_all (),
					   8, byte_order);
      ...
      if (VP16_REGNUM <= regnum && regnum <= VP63_REGNUM)
	{
      ...
      prN = extract_bit_field (pr_val->contents_all (),
                               regnum - VP0_REGNUM, 1);
  ...
  else if (IA64_NAT0_REGNUM <= regnum && regnum <= IA64_NAT31_REGNUM)
    {
      ...
      unat_val = ia64_frame_prev_register (this_frame, this_cache,
                                           IA64_UNAT_REGNUM);
      unatN = extract_bit_field (unat_val->contents_all (),
	  ...
	  if (regnum >= V32_REGNUM)
	    regnum = IA64_GR32_REGNUM + (regnum - V32_REGNUM);
          reg_val = ia64_frame_prev_register (this_frame, this_cache,
                                              IA64_CFM_REGNUM);
	  prev_cfm = extract_unsigned_integer (reg_val->contents_all (),
                                               8, byte_order);
	  reg_val = ia64_frame_prev_register (this_frame, this_cache,
                                              IA64_BSP_REGNUM);
	  prev_bsp = extract_unsigned_integer (reg_val->contents_all (),
                                               8, byte_order);
				    ...
				    void **this_cache, int regnum)
{
  ...
  if (VP0_REGNUM <= regnum && regnum <= VP63_REGNUM)
      ...
      if (VP16_REGNUM <= regnum && regnum <= VP63_REGNUM)
	{
      ...
      prN_val = extract_bit_field (val->contents_all (),
				   regnum - VP0_REGNUM, 1);
  ...
  else if (IA64_NAT0_REGNUM <= regnum && regnum <= IA64_NAT127_REGNUM)
    {
      ...
      unatN_val = extract_bit_field (val->contents_all (),
                                     regnum - IA64_NAT0_REGNUM, 1);
  ...
  else if (regnum == IA64_BSP_REGNUM)
    {
      ...
      prev_bsp = extract_unsigned_integer (val->contents_all (),
					   8, byte_order);
      cfm_val = libunwind_frame_prev_register (this_frame, this_cache,
                                               IA64_CFM_REGNUM);
      prev_cfm = extract_unsigned_integer (cfm_val->contents_all (),
					   8, byte_order);
					     ...
					     void **this_cache, int regnum)
{
  ...
  prev_ip_val = libunwind_frame_prev_register (this_frame, this_cache,
                                               IA64_IP_REGNUM);
  prev_ip = extract_unsigned_integer (prev_ip_val->contents_all (),
				      8, byte_order);
		      ...
		      int struct_return, CORE_ADDR struct_addr)
{
  ...
  for (argno = 0; argno < nargs; argno++)
	...
	{
	  gdb_byte val_buf[8];
	  ULONGEST faddr = extract_unsigned_integer (arg->contents (),
						     8, byte_order);
          ...
          if (!ia64_struct_type_p (type) && len < 8)
            {
              ...
              store_unsigned_integer
                (val_buf, 8, byte_order,
                 extract_unsigned_integer (arg->contents (), len,
					   byte_order));
            }
          else
            {
              ...
              memcpy (val_buf, arg->contents () + argoffset,
                      (len > 8) ? 8 : len);
            }
      ...
      if (float_elt_type != NULL)
	{
	  ...
	  while (len > 0 && floatreg < IA64_FR16_REGNUM)
	    {
	      gdb_byte to[IA64_FP_REGISTER_SIZE];
	      target_float_convert (arg->contents () + argoffset,
				    float_elt_type, to,
