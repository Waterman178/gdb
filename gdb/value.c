			 ...
			 int length)
{
  ...
  gdb_assert (offset1 + length
	      <= TYPE_LENGTH (val1->m_enclosing_type) * TARGET_CHAR_BIT);
  gdb_assert (offset2 + length
	      <= TYPE_LENGTH (val2->m_enclosing_type) * TARGET_CHAR_BIT);
...
allocate_value_contents (struct value *val)
{
  if (!val->m_contents)
	...
	((gdb_byte *) xzalloc (TYPE_LENGTH (val->m_enclosing_type)));
    }
}
...
value::contents_raw ()
{
  struct gdbarch *arch = this->arch ();
  int unit_size = gdbarch_addressable_memory_unit_size (arch);

  allocate_value_contents (this);
  return m_contents.get () + m_embedded_offset * unit_size;
}
			 ...
			 struct value *src, LONGEST src_offset, LONGEST length)
{
  ...
  /* Copy the data.  */
  memcpy (dst->contents_all_raw () + dst_offset * unit_size,
	  src->contents_all_raw () + src_offset * unit_size,
	  length * unit_size);
...
value::optimized_out ()
{
  ...
  if (m_optimized_out.empty () && m_lazy)
	...
	{
	  /* Fall back to checking value->m_optimized_out.  */
	}
      END_CATCH
    }
...
value_copy (struct value *arg)
{
  ...
  if (!val->lazy ())
    {
      memcpy (val->contents_all_raw (), arg->contents_all_raw (),
  ...
  if (arg->lval () != not_lval)
    {
      ...
      memcpy (val->contents_all_raw (), arg->contents_all (),
...
{
  gdb_assert (v->lval () == not_lval);

  write_memory (addr, v->contents_raw (), TYPE_LENGTH (v->type ()));
			   ...
			   LONGEST bitsize, struct value *newval)
{
  ...
  switch (var->kind)
    {
    case INTERNALVAR_VALUE:
      addr = var->u.value->contents_writeable ();
	...
	memcpy (addr + offset * unit_size, newval->contents (),
		TYPE_LENGTH (newval->type ()));
      break;
...
set_internalvar (struct internalvar *var, struct value *val)
{
  ...
  switch (TYPE_CODE (check_typedef (val->type ())))
    {
    case TYPE_CODE_VOID:
    ...
    default:
	 ...
	 leaking memory.  */
      ...
      remove_dyn_prop (DYN_PROP_DATA_LOCATION, new_data.value->type ());
...
{
  gdb_assert (TYPE_CODE (method->type ()) == TYPE_CODE_XMETHOD
	      && method->m_lval == lval_xcallable && argc > 0);

  return method->m_location.xm_worker->get_result_type
...
call_xmethod (struct value *method, int argc, struct value **argv)
{
  ...
  return method->m_location.xm_worker->invoke (argv[0], argv + 1, argc - 1);
}

...
value_as_long (struct value *val)
{
  ...
  return unpack_long (val->type (), val->contents ());
}
...
{
  struct gdbarch *gdbarch = get_type_arch (val->type ());
  ...
  if (TYPE_CODE (val->type ()) != TYPE_CODE_PTR
      && !TYPE_IS_REFERENCE (val->type ())
...
is_floating_value (struct value *val)
{
  ...
  if (is_floating_type (type))
    {
      if (!target_float_is_valid (val->contents (), type))
  ...
  if (TYPE_FIELD_BITSIZE (arg_type, fieldno))
  ...
  else if (fieldno < TYPE_N_BASECLASSES (arg_type))
    {
	 ...
	 bases, etc.  */
      ...
      if (arg1->lval () == lval_register && arg1->lazy ())
	arg1->fetch_lazy ();
  ...
  if (valaddr != NULL)
    {
      ...
      num = unpack_bits_as_long (field_type, valaddr + embedded_offset,
				 bitpos, bitsize);
      store_signed_integer (dest_val->contents_raw (),
...
value_from_longest (struct type *type, LONGEST num)
{
  ...
  pack_long (val->contents_raw (), type, num);
  return val;
}
...
value_from_ulongest (struct type *type, ULONGEST num)
{
  ...
  pack_unsigned_long (val->contents_raw (), type, num);
...
value_from_pointer (struct type *type, CORE_ADDR addr)
{
  ...
  store_typed_address (val->contents_raw (),
...
value_from_contents (struct type *type, const gdb_byte *contents)
{
  ...
  memcpy (result->contents_raw (), contents, TYPE_LENGTH (type));
  return result;
}
...
value::fetch_lazy_register ()
{
  ...
  if (frame_debug)
	...
	{
	  int i;
	  const gdb_byte *buf = new_val->contents ();
		    ...
		    void *cookie, int argc, struct value **argv)
{
  ...
  ret = TYPE_CODE (argv[0]->type ()) == TYPE_CODE_VOID;
